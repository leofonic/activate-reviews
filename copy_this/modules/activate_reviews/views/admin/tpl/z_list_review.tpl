[{include file="headitem.tpl" title="SHOWLIST_TITLE"|oxmultilangassign box="list"}]
...
function editThis ( sID )
{
    ...
    var oTransfer = document.getElementById("editarticle");
...
<form name="editarticle" target="basefrm" id="editarticle" action="[{ $oViewConf->getSelfLink() }]" method="post">
...
<form name="transfer" id="transfer" action="[{ $oViewConf->getSelfLink() }]" method="post">
    ...
    <input type="hidden" name="rev_oxid" value="">
    <input type="hidden" name="language" value="[{ $actlang }]">
    <input type="hidden" name="editlanguage" value="[{ $actlang }]">
</form>
<form name="search" id="search" action="[{ $oViewConf->getSelfLink() }]" method="post">
    [{ $oViewConf->getHiddenSid() }]
    <input type="hidden" name="cl" value="z_list_review">
...
[{ foreach from=$mylist item=oReview }]
    ...
    [{ if $oReview->oxreviews__oxid->value == $oxid }]
        [{assign var="listclass" value="listitem4" }]
    [{ /if}]
    
    <tr id="row.[{$_cnt}]">
        [{block name="admin_list_review_item"}]
            <td align="center" class="[{ $listclass }]" valign="top"><a href="Javascript:top.oxid.admin.editThis('[{$oReview->oxreviews__oxid->value}]');" class="[{ $listclass }]">[{ $oReview->oxreviews__oxcreate|oxformdate }]</a></td>
            <td class="[{ $listclass }]" valign="top"><a href="Javascript:top.oxid.admin.editThis('[{$oReview->oxreviews__oxid->value}]');" class="[{ $listclass }]">[{ $oReview->oxreviews__oxtext->value }]</a></td>
            <td class="[{ $listclass }]" valign="top"><a href="Javascript:editThis( '[{$oReview->oxreviews__oxobjectid->value}]');" class="[{ $listclass }]">[{if $oReview->oxreviews__oxparentid->value}][{ $oReview->oxreviews__parenttitle->value }] [{ $oReview->oxreviews__oxvarselect->value }][{else}][{$oReview->oxreviews__oxtitle->value}][{/if}]</a></td>
        [{/block}]
    </tr>
[{if $blWhite == "2"}]
    [{assign var="blWhite" value=""}]
